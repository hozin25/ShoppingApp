...
实施 `Redis预扣减 + RabbitMQ消息队列` 方案来防止商品超卖，支持高并发场景（QPS > 10000）。该方案通过Redis快速预扣减库存拦截大部分请求，使用消息队列削峰填谷异步处理订单，最终通过数据库乐观锁保证数据一致性。

## Current State Analysis

### 当前实现存在的并发问题

**文件**: [`server/src/main/java/com/controller/ShangpinOrderController.java:431-528`](https://github.com/hozin25/ShoppingApp/blob/master/server/src/main/java/com/controller/ShangpinOrderController.java#L431-L528)
...
- ❌ 不修改数据库表结构（不加version字段，使用数据库锁机制代替）
- ❌ 不实现复杂的分布式事务（使用最终一致性）
...
public class OrderConsumer {
    ...
    private boolean processOrderInternal(OrderMessage message) {
        try {
            ...
            // 4. 创建订单记录
            R result = shangpinOrderService.createOrderFromMessage(message);
...
**File**: `server/src/main/java/com/service/ShangpinOrderService.java` (修改)
 ...
 * @return 处理结果
 */
R createOrderFromMessage(OrderMessage message);
```

**File**: `server/src/main/java/com/service/impl/ShangpinOrderServiceImpl.java` (修改)
...
@Override
@Transactional
public R createOrderFromMessage(OrderMessage message) {
    try {
        // 创建订单实体
        ...
        // 插入订单
        boolean insertSuccess = this.insert(order);
...
**File**: `server/src/main/java/com/service/OrderTrackingService.java` (新建)
...
public interface OrderTrackingService {
     ...
     * 保存订单处理记录
     */
    void saveOrderRecord(OrderMessage message, String status, String remark);

    /**
     * 更新订单状态
     */
    void updateOrderStatus(String orderUuid, String status);

    /**
     * 获取订单状态
     */
    String getOrderStatus(String orderUuid);
}
```
...
**File**: `uni-mall/pages/shangpinOrder/list.vue` (修改)
...
methods: {
    async checkOrderStatus() {
        ...
        for (let order of this.list) {
            if (order.shangpinOrderTypes === 'processing') {
                let res = await this.$api.info('shangpinOrder', order.id);
                if (res.data.shangpinOrderTypes !== 'processing') {
                    // 状态已更新，刷新列表
                    this.getOrderList();
                    break;
                }
...
public class InventoryServiceTest {
    ...
    public void testDeductInventory() {
        // 初始化库存
        inventoryService.initInventoryToRedis(1001, 100);

        // 扣减库存
        boolean success = inventoryService.deductInventory(1001, 10);
        assertTrue(success);

        // 验证库存
        Integer quantity = inventoryService.getInventoryFromRedis(1001);
        assertEquals(90, quantity);
    }

    @Test
    public void testDeductInsufficientInventory() {
        // 初始化库存
        inventoryService.initInventoryToRedis(1002, 5);

        // 尝试扣减更多库存
        boolean success = inventoryService.deductInventory(1002, 10);
        assertFalse(success);

        // 验证库存不变
        Integer quantity = inventoryService.getInventoryFromRedis(1002);
        assertEquals(5, quantity);
    }

    @Test
    public void testRollbackInventory() {
        // 初始化库存
        inventoryService.initInventoryToRedis(1003, 100);

        // 扣减库存
        inventoryService.deductInventory(1003, 10);

        // 回滚库存
        boolean success = inventoryService.rollbackInventory(1003, 10);
        assertTrue(success);

        // 验证库存恢复
        Integer quantity = inventoryService.getInventoryFromRedis(1003);
