...
//    	ValidatorUtils.validateEntity(user);
    ...
    public R resetPass(@RequestBody Map<String, String> params) {
        try {
            String username = params.get("username");
            String yonghuPhone = params.get("yonghu_phone");
            String tableName = params.get("tableName");
            
            if (StringUtils.isEmpty(username) || StringUtils.isEmpty(yonghuPhone)) {
                return R.error("账号和手机号不能为空");
            }
            ...
            // 验证手机号是否匹配
            if (!yonghuPhone.equals(user.getYonghuPhone())) {
                return R.error("手机号与注册信息不符");
            }

            // 生成新密码并加密后存储
            String newPass = String.format("%06d", new Random().nextInt(1000000));
            user.setPassword(DigestUtils.md5Hex(newPass));
            
            boolean updated = yonghuService.updateById(user);
            if (!updated) {
                return R.error("密码重置失败");
            }
            
            return R.ok("新密码是:" + newPass);
        } catch (Exception e) {
            e.printStackTrace();
...
//            .notIn("yonghu_types", new Integer[]{102})
    ...
    public R upgradePasswords() {
        try {
            List<YonghuEntity> users = yonghuService.selectList(null);
            int count = 0;
