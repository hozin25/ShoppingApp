...
//            shangpinOrder.setYonghuId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
    ...
    */
    ...
    public R delete(@RequestBody Integer[] ids, HttpServletRequest request){
        ...
        // 查询要处理的订单
        List<ShangpinOrderEntity> oldShangpinOrderList = shangpinOrderService.selectBatchIds(Arrays.asList(ids));
        
        // 处理每个订单
        for(ShangpinOrderEntity order : oldShangpinOrderList) {
            // 只处理已支付的订单
            if(order.getShangpinOrderTypes() == 101) {
                // 获取用户信息
                YonghuEntity yonghu = yonghuService.selectById(order.getYonghuId());
                if(yonghu != null) {
                    // 退还订单金额给用户
                    yonghu.setNewMoney(yonghu.getNewMoney() + order.getShangpinOrderTruePrice());
                    yonghuService.updateById(yonghu);
                }
                
                // 获取商品信息
                ShangpinEntity shangpin = shangpinService.selectById(order.getShangpinId());
                if(shangpin != null) {
                    // 获取商家信息
                    ShangjiaEntity shangjia = shangjiaService.selectById(shangpin.getShangjiaId());
                    if(shangjia != null) {
                        // 从商家账户扣除订单金额
                        shangjia.setNewMoney(shangjia.getNewMoney() - order.getShangpinOrderTruePrice());
                        shangjiaService.updateById(shangjia);
                    }
                    
                    // 恢复商品库存
                    shangpin.setShangpinKucunNumber(shangpin.getShangpinKucunNumber() + order.getBuyNumber());
                    shangpinService.updateById(shangpin);
                }
                
                // 更新订单状态为已退款(102)
                order.setShangpinOrderTypes(102);
